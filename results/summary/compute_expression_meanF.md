...
barcode_runs <- barcode_runs[barcode_runs$experiment_type == "SortSeq",]
...
counts <- subset(counts, sample %in% barcode_runs[barcode_runs$experiment_type=="SortSeq","sample"])
counts <- counts[, c("library", "barcode", "sample", "count")] 

#read in barcode-variant lookup tables
dt <- data.table(read.csv(file=config$codon_variant_table,stringsAsFactors=F))

# merge with barcode counts 
...
```
    ...
    ## [1] "read:cell ratio for lib1 SortSeq_bin1 is 2.09136727688787"
    ## [1] "read:cell ratio for lib1 SortSeq_bin2 is 1.85518152380952"
    ## [1] "read:cell ratio for lib1 SortSeq_bin3 is 2.73451947731755"
    ## [1] "read:cell ratio for lib1 SortSeq_bin4 is 2.20928349282297"
    ## [1] "read:cell ratio for lib2 SortSeq_bin1 is 2.17348262548263"
    ## [1] "read:cell ratio for lib2 SortSeq_bin2 is 2.72144971453667"
    ## [1] "read:cell ratio for lib2 SortSeq_bin3 is 2.07171950617284"
    ## [1] "read:cell ratio for lib2 SortSeq_bin4 is 2.21752578475336"
...
invisible(dt[library=="lib1",c("expression","ML_sdF") := tryCatch(calc.MLmean(b1=SortSeq_bin1*20,b2=SortSeq_bin2*20,
                                                                      ...
                                                                      min.b1=log(20),min.b2=log(464),min.b3=log(8228),
                                                                      min.b4=log(23650),max.b4=log(229000)),
...
invisible(dt[library=="lib2",c("expression","ML_sdF") := tryCatch(calc.MLmean(b1=SortSeq_bin1*20,b2=SortSeq_bin2*20,
                                                                      b3=SortSeq_bin3*20,b4=SortSeq_bin4*20,
                                                                      min.b1=log(20),min.b2=log(459),min.b3=log(8307),
                                                                      min.b4=log(24049),max.b4=log(229000)),
                                                          error=function(e){return(list(as.numeric(NA),as.numeric(NA)))}),by=c("library","barcode")])
...
``` r
#save pdf
invisible(dev.print(pdf, paste(config$expression_sortseq_dir,"/violin-plot_meanF.pdf",sep="")))
```

We have generated expression measurements for 94.37% of the barcodes in
our libraries.
