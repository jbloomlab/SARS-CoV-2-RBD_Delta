...
```python
os.makedirs(config['early2020_escape_profiles_dir'], exist_ok=True)
```
...
```python
site_metric = config['early2020_site_metric']
mut_metric = config['early2020_mut_metric']
...
```python
print(f"Reading sites of strong escape from {config['early2020_strong_escape_sites']}")

strong_escape_sites = pd.read_csv(config['early2020_strong_escape_sites'])
```

    Reading sites of strong escape from results/early2020_escape_profiles/strong_escape_sites.csv
...
```python
print(f"Reading escape fractions from {config['early2020_escape_fracs']}")
escape_fracs = (pd.read_csv(config['early2020_escape_fracs'])
...
```
    ...
    Reading escape fractions from results/prior_DMS_data/early2020_escape_fracs.csv
...
wt_prot = str(Bio.SeqIO.read(config['early2020_wildtype_sequence'], 'fasta').seq.translate())
...
```python
# this is going to need to be changed in the future! 
mut_bind_expr_file = config['early2020_mut_bind_expr']
...
for prop in ['bind', 'expr']:
    ...
    print(f"Saving DMS color scheme to {config['early2020_escape_profiles_dms_colors']}")
    (mut_bind_expr
     .query('escape_gt_0')
     .drop(columns='escape_gt_0')
     .to_csv(config['early2020_escape_profiles_dms_colors'], index=False)
     )
...
```
    ...
    Reading DMS data from results/prior_DMS_data/early2020_mutant_ACE2binding_expression.csv
...
![png](early2020_escape_profiles_files/early2020_escape_profiles_18_1.png)
...
![png](early2020_escape_profiles_files/early2020_escape_profiles_18_3.png)
    


    Saving DMS color scheme to results/escape_profiles/early2020_escape_profiles_dms_colors.csv
...
![png](early2020_escape_profiles_files/early2020_escape_profiles_18_5.png)
...
![png](early2020_escape_profiles_files/early2020_escape_profiles_18_7.png)
...
```python
print(f"Reading escape-profile configuration from {config['early2020_escape_profiles_config']}")
with open(config['early2020_escape_profiles_config']) as f:
...
```
    ...
    Reading escape-profile configuration from data/early2020_escape_profiles_config.yaml
...
for name, specs in escape_profiles_config.items():
    ...
    # write the ylimits
    ylims_csv = os.path.join(config['early2020_escape_profiles_dir'], f"{name}_stackedlogo_ylims.csv")
    (
    ...
    colors_plotfiles = [(color_col, os.path.join(config['early2020_escape_profiles_dir'], f"{name}_stackedlogo.pdf"))]
    if 'color_by_dms' in specs and specs['color_by_dms']:
        colors_plotfiles += [('bind_color', os.path.join(config['early2020_escape_profiles_dir'], f"{name}_color_by_bind_stackedlogo.pdf")),
                             ('expr_color', os.path.join(config['early2020_escape_profiles_dir'], f"{name}_color_by_expr_stackedlogo.pdf"))]
...
```
    ...
    Drawing 4 profiles using 8 CPUs...
    
    Plotted profile 1 to:
     results/early2020_escape_profiles/haarvi_sera_stackedlogo.pdf
     results/early2020_escape_profiles/haarvi_sera_stackedlogo.png.



    
![png](early2020_escape_profiles_files/early2020_escape_profiles_26_1.png)
...
![png](early2020_escape_profiles_files/early2020_escape_profiles_26_3.png)
    ...
    Plotted profile 3 to:
     ...
     results/early2020_escape_profiles/haarvi_sera_color_by_expr_stackedlogo.png.
