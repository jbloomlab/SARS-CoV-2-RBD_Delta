...
combined_df = (pd.concat([
    (neut_titers.query("replicate=='average'")
     ...
     .query('virus!="D614G RBD Abs depleted (x D614G PV)"')
    ),
    moderna,
    infection
]
)
                .assign(background=lambda x: pd.Categorical(x['background'],
                                                 ordered=True,
...
```python
df=(combined_df
    .query("virus in @config['combined_plot']")
    .assign(virus=lambda x: pd.Categorical(x['virus'],
                                               ordered=True,
                                               categories=config['combined_plot']),
...
```
    ...
    (17.999999999999996, 40495.49949040342)
...
p = (ggplot(df
     ...
     theme_classic() +
     theme(axis_title_x=element_blank(),
           figure_size=(df['virus'].nunique()*df['background'].nunique()*0.35, 2.5),
           axis_text_x=element_text(rotation=90),
           strip_background_x=element_blank(),
           ) +
